...
//	characters="!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[]^_`abcdefghijklmnopqrstuvwxyz{|}~¡¢£¤¥¦§¨©ª«¬®¯°±²³´µ¶·¸¹º»¼½¾ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕ×ßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûü",
	...
	compressor={
		...
		get(array){
			return array.sort((one,two)=>compressor.sort(one)-compressor.sort(two))[0];
		},
		map(array,base){
		...
		update(){
			...
			fields.compressor.value+=`Compressed:  ${compressor.result.string}\n`;
			if(compressor.result.base)
			...
			if(compressor.permute)
				compressor.buttons.reset.addEventListener(`animationiteration`,()=>compressor.buttons.reset.classList.remove(`spin`),false);
		}
	},
	docs={
		sidebar:I(`docs`),
	...
	general={
		...
		listeners(){
			...
			I(`run`).addEventListener(`click`,()=>!interpreter.running?interpreter.run():interpreter.stop(),false);
			...
			compressor.buttons.permute.addEventListener(`click`,()=>compressor.buttons.permute.classList.toggle(`enabled`,compressor.permute=!compressor.permute),false);
