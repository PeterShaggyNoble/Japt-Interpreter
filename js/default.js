{
	const 	W=window,
		...
		characters="$[]^_`{|}~¡¢£¤¥¦§¨©ª«¬®¯°±²³´µ¶·¸¹º»¼½¾ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûü",
		variables={
			bytes:0,
			running:false,
			safemode:false
		...
		version={
			current:`1.4.6`,
			numbers:[`1.3`,`1.4`,`1.4.1`,`1.4.2`,`1.4.3`,`1.4.4`,`1.4.5`,`1.4.6`,`2.0a0`]
		},
		charmap=()=>{
			let key,li;
		...
		getlink=()=>{
			let output=href.protocol+`//${href.hostname+href.pathname}?v=`+version.selected;
			if(fields.flags.value)
				output+=`&flags=`+encode(fields.flags.value);
			if(fields.code.value)
				output+=`&code=`+encode(fields.code.value);
			if(fields.input.value)
				output+=`&code=`+encode(fields.input.value);
			return output;
		},
		grow=nodes=>{
		...
		init=()=>{
			...
			seticons();
			charmap();
			let url=`https://cdn.jsdelivr.net/gh/ETHproductions/japt@${version.selected===version.current||version.two?`master`:version.selected}/src/${version.two?`japt`:`japt-interpreter`}.js`;
			loadjs(url).then(()=>{
		...
		listeners=()=>{
			...
			buttons.link.addEventListener(`click`,()=>copy(getlink()),false);
			...
			fields.input.addEventListener(`input`,()=>grow([fields.input]),false);
			...
			W.addEventListener(`resize`,()=>grow([fields.code,fields.transpiled,fields.input,fields.output]),false);
		},
		loadjs=url=>new Promise((resolve,reject)=>{
			let script=E(`script`);
			script.async=true;
			script.src=url;
			B.append(script);
		...
		seticons=()=>{
			...
			for(svg of A(`svg[data-mdi]`)){
				...
				for(icon of svg.dataset.mdi.split(`,`)){
					...
					path.setAttribute(`d`,icons[icon]||icons[`help-circle-outline`]);
					svg.append(path);
				}
			}
		},
		setup=()=>{
			...
			Q(`title`).firstChild.nodeValue+=version.selected;
		...
		update=(...nodes)=>{
			let 	code=fields.code.value,
				encoding=`ISO-8859-1`
			fields.transpiled.value=(W.Japt||W).transpile(code);
			grow(nodes);
			...
			fields.counter.firstChild.nodeValue=variables.bytes+` bytes`.slice(0,code.length>1?6:5)+` (${encoding})`;
		},
		icons={
